...
class data_helper:
	...
	def get_dataset(self, bucket_size=[], token_length=512, min_first_sentence_length=3, min_second_sentence_length=5):
			...
			for sentence_index in range(len(first)):
				...
				if is_next == 0: # not next
					...
					while True:
						other_paragraph_index = self.exclusive_randint(
								min_val=0, 
								max_val=paragraph_num, 
								exclusive_vals=[paragraph_index]+zero_sentence_paragraph_index
							)
						other_first, other_second = self.get_sentence_from_paragraph(
								other_paragraph_index, 
								token_length=token_length,
						...
						if len(other_first):
							...
							zero_sentence_paragraph_index.append(other_paragraph_index)
							#count+=1
							#print('hi', count)
					...
					if len(possible_other_sentence_index): #길이를 만족하는것이 있는 경우
						...
						random_other_sentence_index = np.random.randint(0, len(other_sentences))
						second_sentence = other_sentences[random_other_sentence_index][:len(second[sentence_index])] #[:token_length-3-first_sentence_length]
					...
					second_sentence = second[sentence_index] # >= min_second_sentence_length


				# for masked language model task
				concat_sentences_for_mask = np.array(first_sentence+second_sentence, dtype=np.int32)
				concat_sentences_for_mask_length = len(concat_sentences_for_mask) 
				#if concat_sentences_for_mask_length+3 < 20:
				#	continue

				total_token += concat_sentences_for_mask_length
				...
				len_check[len(first_sentence) + len(second_sentence) + 3] += 1
				'''
		...
		'''
				...
				#print(len(complete_sentence), len(complete_mask))
				try:
					len_check[len(complete_sentence)] += 1
				except:
		...
		for paragraph_index in tqdm(range(paragraph_num), ncols=50):
			...
			for sentence_index in range(1, paragraph_length):
				...
				if first_sentence_length + 3 + min_second_sentence_length >= token_length: # second sentence 길이가 최대 5밖에 안되면 스킵
					continue

				# for is_next sentence prediction
				is_next = np.random.randint(0, 2) # 0: not next, 1: next
				if is_next == 0: # not next
					other_paragraph_index = self.exclusive_randint(min_val=0, max_val=paragraph_num, exclusive_vals=[paragraph_index])
					other_paragraph = self.data[other_paragraph_index]
					possible_sentence_index = []
					for other_sentence_index, other_sentence in enumerate(other_paragraph):
						if len(other_sentence) + first_sentence_length + 3 <= token_length: # 길이를 만족하는 경우
							possible_sentence_index.append(other_sentence_index)
					if len(possible_sentence_index):
						second_sentence = other_paragraph[np.random.choice(possible_sentence_index, 1)[0]]
					else: #길이를 만족하는 것이 없으면 아무 문장이나 선택하고 slice 함.
						random_index = np.random.randint(0, len(other_paragraph)) # 0: not next, 1: next
						second_sentence = other_paragraph[random_index][:token_length-3-first_sentence_length]
				else: #is next
					second_sentence = paragraph[sentence_index]
					if first_sentence_length + 3 + len(second_sentence) > token_length:
						continue
